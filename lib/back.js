    ...
    if (context.isRecording) {
      ...
      try {
        outputs = outputs.map(function(output) {
          if (options.recorder.gunzip && output.rawHeaders.includes('gzip')) {
            debug('re-gzipping response')

            return {
              ...output,
              response: zlib.gzipSync(
                Buffer.from(JSON.stringify(output.response))
              ),
            }
          }
        })
        ...
        throw new Error(
          `Error occurred while gzipping response: ${err.message}`
        )
      }
