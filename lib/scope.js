...
function Scope(basePath, options) {
  ...
  if (!(basePath instanceof RegExp)) {
    ...
    var protocolFound = null;
    if (protocolDelim.test(basePath)) {
      let split = basePath.split(protocolDelim);
      protocolFound = split[0];
      basePath = split[1];
      this.urlParts.protocol = protocolFound;
    };

    // Extract host and pathname
    var urlNoProtocol = basePath.split(/\/(.+)/);
    var host = urlNoProtocol[0];
    host = host.replace(/\/$/, '');

    var pathname = urlNoProtocol[1];
    if (pathname) {
      this.urlParts.pathname = '/' + pathname;
    } else {
      this.urlParts.pathname = null;
    }

    // Search for port in host
    hostSplitted = host.split(':')
    this.urlParts.hostname = hostSplitted[0];

    // No port found
    if(hostSplitted.length < 2) {
      if (protocolFound) {
        this.urlParts.port = protocolFound === 'https:' ? 443 : 80;
      } else {
        this.urlParts.protocol = 'http:';
        this.urlParts.port = 80
      }
    } else {
      this.urlParts.port = hostSplitted[1];

      if(!protocolFound) {
        this.urlParts.protocol = this.urlParts.port === 443 ? 'https:' : 'http:';
      }
    }

    this.urlParts.host = this.urlParts.hostname + ':' + this.urlParts.port;
    this.basePath = this.urlParts.protocol + '//' + this.urlParts.host;

    if(this.urlParts.pathname) {
      this.basePath += this.urlParts.pathname;
    }
  }
}
