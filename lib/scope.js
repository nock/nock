...
function startScope(basePath, options) {
  if (options && (options.legacy === false || options.legacy === true)) {
    // Re-activate nock intercept with correct options if legacy was passed
    globalIntercept.restoreOverriddenClientRequest()
    common.restoreOverriddenRequests()
    globalIntercept.activate({ legacy: options.legacy })
  }
...
function Scope(basePath, options) {
  ...
  this.logger = debug
  this.scopeOptions =
    options && (options.legacy === false || options.legacy === true)
      ? Object.assign({}, options)
      : Object.assign({}, options, { legacy: globalIntercept.LEGACY })
  this.urlParts = {}
  this._persist = false
