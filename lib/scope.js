...
function startScope(basePath, options) {
  var interceptors = {},
      ...
      persist = false,
      af_unix = options && options.af_unix;

  if (!af_unix) {
    var urlParts = url.parse(basePath);
    var port = urlParts.port || ((urlParts.protocol === 'http:') ? 80 : 443);
  ...
  function intercept(uri, method, requestBody, interceptorOptions) {
    ...
    function match(options, body, hostNameOnly) {
      if (hostNameOnly && urlParts) return options.hostname === urlParts.hostname;
