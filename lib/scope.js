...
function getScopeFromDefinition(nockDef) {
  ...
  if (!_.isUndefined(nockDef.port)) {
    ...
    if (_.isNull(options.port)) {
      ...
      // TODO-coverage: Add a test for the `else` case: when a port is
      // specified and there's also a port in the URL, but they match.
...
function define(nockDefs) {
  ...
  nockDefs.forEach(function(nockDef) {
    ...
    // TODO-coverage: Throw an error when `method` is missing.
