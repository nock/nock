...
function overrideClientRequest() {
  ...
  const originalClientRequest = getOriginalModuleProp(http, 'ClientRequest')
  ...
  inherits(OverriddenClientRequest, originalClientRequest)
...
function storeOriginalModulePropOnModule(module, method) {
  /*
   Because of the following, we need to store the originals on the http(s) modules instances themselves:
   1. the non-native module-cache is being reset when using jest (so the `nock` and `@msw/interceptors` modules are being re-initialised, but not `node:http` and `node:https`)
   2. `globalThis` is also being reset when using jest-environment-node
   3. MSWs ClientRequestInterceptor is proxying the http(s) modules: https://github.com/mswjs/interceptors/blob/main/src/interceptors/ClientRequest/index.ts#L34
   4. nock needs `http(s).*` to be returned to the original state so nock and MSW are in sync with their patching
   Any other way of storing the originals will be reset between test-files (and so each nock module being imported/required),
    and so the originals will be lost.
   See this issue: https://github.com/nock/nock/issues/2802
   Equiv PR in MSW: https://github.com/mswjs/interceptors/pull/697
  */
  if (!module.__nock_originalProps) {
    module.__nock_originalProps = {}
  }
  if (!module.__nock_originalProps[method]) {
    debug('writing original method "%s" to <module>.__nock_originalProps', method)
    module.__nock_originalProps[method] = module[method]
  } else {
    debug('original method "%s" already exists on <module>.__nock_originalProps', method)
  }
}

function getOriginalModuleProp(module, method) {
  if (module.__nock_originalProps?.[method]) {
    debug('returning original method "%s" from <module>.__nock_originalProps', method)
    return module.__nock_originalProps[method]
  }
  debug('no original method "%s" found on <module>.__nock_originalProps (taking from module)', method)
  return module[method]
}

function storeOriginalClientRequest() {
  debug('storing original client request methods on http(s) modules')
  storeOriginalModulePropOnModule(http, 'request')
  storeOriginalModulePropOnModule(http, 'get')
  storeOriginalModulePropOnModule(http, 'ClientRequest')
  storeOriginalModulePropOnModule(https, 'request')
  storeOriginalModulePropOnModule(https, 'get')
}

storeOriginalClientRequest()

function restoreModuleProp(module, method) {
  if (module.__nock_originalProps?.[method]) {
    module[method] = module.__nock_originalProps[method]
    return true
  }
  return false
}
...
function restoreOverriddenClientRequest() {
  debug('restoring overridden http(s) methods')

  debug(`- http.ClientRequest original ${ restoreModuleProp(http, 'ClientRequest') ? 'restored' : 'not found'}`)
  debug(`- http.request original ${ restoreModuleProp(http, 'request') ? 'restored' : 'not found'}`)
  debug(`- http.get original ${ restoreModuleProp(http, 'get') ? 'restored' : 'not found'}`)
  debug(`- https.request original ${ restoreModuleProp(https, 'request') ? 'restored' : 'not found'}`)
  debug(`- https.get original ${ restoreModuleProp(https, 'get') ? 'restored' : 'not found'}`)
  delete http.__nock_originalProps
  delete https.__nock_originalProps
