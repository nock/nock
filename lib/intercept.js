...
function addInterceptor(key, interceptor, scope, scopeOptions, host) {
  if (!(key in global.allInterceptors)) {
    global.allInterceptors[key] = { key, interceptors: [] }
  }
  ...
  if (scopeOptions.allowUnmocked) global.allInterceptors[key].allowUnmocked = true

  global.allInterceptors[key].interceptors.push(interceptor)
}
...
function remove(interceptor) {
  ...
  const interceptors =
    (global.allInterceptors[basePath] && global.allInterceptors[basePath].interceptors) || []
...
function removeAll() {
  Object.keys(global.allInterceptors).forEach(function (key) {
    global.allInterceptors[key].interceptors.forEach(function (interceptor) {
...
function removeInterceptor(options) {
  ...
  if (
    global.allInterceptors[baseUrl] &&
    global.allInterceptors[baseUrl].interceptors.length > 0
  ) {
    for (let i = 0; i < global.allInterceptors[baseUrl].interceptors.length; i++) {
      const interceptor = global.allInterceptors[baseUrl].interceptors[i]
      if (
        options instanceof Interceptor
          ? interceptor === options
          : interceptor._key === key
      ) {
        global.allInterceptors[baseUrl].interceptors.splice(i, 1)
...
function interceptorScopes() {
  const nestedInterceptors = Object.values(global.allInterceptors).map(
    i => i.interceptors
  )
...
function activate() {
  ...
  common.overrideRequests(function (proto, overriddenRequest, args) {
    ...
    const { options, callback } = proto === 'msw'
      ? { options: getRequestOptionsFromFetchRequest(args.request), callback: args.callback }
    ...
    if (proto != 'msw') options.proto = proto
