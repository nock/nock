...
function interceptorsFor(options) {
  ...
  _.each(allInterceptors, function(interceptor, k) {
    ...
    if (matchingInterceptor && matchingInterceptor.length) {
      matchingInterceptor = matchingInterceptor.filter(function(scope) {
        const { filteringExternal } = scope.__nock_scopeOptions || {}

        return !filteringExternal || filteringExternal(basePath)
      })

      if (!matchingInterceptor.length) {
        // move on to the next intercept if all the scopes get filtered
        matchingInterceptor = null
      }
    }
