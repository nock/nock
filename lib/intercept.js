...
function storeOriginalModulePropOnModule(module, method) {
  ...
  */
  ...
  if (!module.__nock_originalProps[method]) {
    debug(
      'writing original method "%s" to <module>.__nock_originalProps',
      method,
    )
    module.__nock_originalProps[method] = module[method]
  } else {
    debug(
      'original method "%s" already exists on <module>.__nock_originalProps',
      method,
    )
  }
}
...
function getOriginalModuleProp(module, method) {
  if (module.__nock_originalProps?.[method]) {
    debug(
      'returning original method "%s" from <module>.__nock_originalProps',
      method,
    )
    return module.__nock_originalProps[method]
  }
  debug(
    'no original method "%s" found on <module>.__nock_originalProps (taking from module)',
    method,
  )
...
function restoreOverriddenClientRequest() {
  ...
  debug(
    `- http.ClientRequest original ${restoreModuleProp(http, 'ClientRequest') ? 'restored' : 'not found'}`,
  )
  debug(
    `- http.request original ${restoreModuleProp(http, 'request') ? 'restored' : 'not found'}`,
  )
  debug(
    `- http.get original ${restoreModuleProp(http, 'get') ? 'restored' : 'not found'}`,
  )
  debug(
    `- https.request original ${restoreModuleProp(https, 'request') ? 'restored' : 'not found'}`,
  )
  debug(
    `- https.get original ${restoreModuleProp(https, 'get') ? 'restored' : 'not found'}`,
  )
