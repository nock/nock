    ...
    return new Promise((resolve, reject) => {
      ...
      if (isOn() && interceptors) {
        ...
        if (!matches && allowUnmocked) {
          ...
          // TODO: we don't want to reject, but propagate the error via the response
          nockRequest.on('error', reject)
        ...
        if (isOff() || isEnabledForNetConnect(options)) {
          ...
          reject(new NetConnectNotAllowedError(options.host, options.path))
        }
      }
    })
  })
