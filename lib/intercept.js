...
function isActive() {
  // TODO: should mswjs/interceptors expose such API?
  return isNockActive
}
...
function activate() {
  ...
  interceptor.apply();
  interceptor.on('request', function ({ request: mswRequest }) {
      ...
      if (isOn() && interceptors) {
        ...
        if (!matches && allowUnmocked) {
          ...
          nockRequest.on('response', nockResponse => {
            ...
            mswRequest.respondWith(response)
            resolve()
          })
          nockRequest.on('error', function () { 
            // TODO: https://github.com/mswjs/interceptors/issues/492
            setTimeout(reject, 20, ...arguments) 
          })
          // TODO: temp hacky way to handle allowUnmocked in startPlayback
          nockRequest.on('real-request', resolve)

          // TODO: improve this. any ideas are welcomed
          request.arrayBuffer()
            .then(nockRequest.write)
            // TODO DONT MERGE BEFORE FIX: check unchought error here:
            .finally(nockRequest.end)
        }
      } else {
        ...
        if (isOff() || isEnabledForNetConnect(options)) {
          ...
          // TODO: https://github.com/mswjs/interceptors/issues/492
          setTimeout(reject, 20, new NetConnectNotAllowedError(options.host, options.path)) 
        }
      }
    })
  })
