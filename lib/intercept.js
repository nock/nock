...
const { BatchInterceptor, InterceptorReadyState, getRawRequest, FetchResponse } = require('@mswjs/interceptors')
  ...
  interceptor.on('request', async function ({ request, controller }) {
    ...
    if (isOn() && interceptors) {
      ...
      if (!matches && allowUnmocked) {
        ...
        const decompressedRequest = new Request(request, { body: requestBodyBuffer.length > 0 && request.method !== 'GET' ? requestBodyBuffer : undefined })
        ...
        const matchedInterceptor = interceptors.find(i =>
          i.match(decompressedRequest, requestBodyString),
        )
