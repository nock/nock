...
function DelayedBody(ms, body) {
  ...
  if (common.isStream(body)) {
    body.on('data', function(chunk) {
      // TODO-coverage: Probably needs a test with `delayBody()` and a binary
      // response?
  ...
  setTimeout(function() {
    // TODO-coverage: This seems to be covering a case where the string takes
    // longer to process than the minimum `ms` delay passed into
    // DelayedBody(). This looks like a challenge!
