...
const { extractRawHeaders } = require('../common')
    ...
    handleRequest(decompressedRequest)
      .then(async response => {
        if (response) {
          ...
          handler.onHeaders?.(response.status, extractRawHeaders(response.headers), () => {}, response.statusText)
          handler.onData?.(Buffer.from(await response.arrayBuffer()))
          handler.onComplete?.([]) // responseTrailers
        } else {
          const dispatcher = options.dispatcher || { dispatch: super.dispatch.bind(this) }
