...
var getBodyFromChunks = function(chunks, headers, gunzip) {
  if (gunzip && common.isGZipped(headers)) {
    debug('unzipping gzipped response');

    try {
      var decoded = zlib.gunzipSync(Buffer.concat(chunks)).toString()
    } catch (err) {
      throw new Error('error unzipping response body\n' + err)
    }

    var result = isJsonString(decoded) ? JSON.parse(decoded) : decoded
...
function generateRequestAndResponseObject(req, bodyChunks, options, res, dataChunks, gunzip) {

  var response = getBodyFromChunks(dataChunks, res.headers, gunzip)
...
function generateRequestAndResponse(req, bodyChunks, options, res, dataChunks, gunzip) {

  var requestBody = getBodyFromChunks(bodyChunks, null, gunzip).body;
  var responseBody = getBodyFromChunks(dataChunks, res.headers, gunzip).body;
  ...
  common.overrideRequests(function(proto, overriddenRequest, options, callback) {
    ...
    var req = overriddenRequest(options, function(res) {
      ...
      res.once('end', function() {
        ...
        var out;
        if(output_objects) {
          out = generateRequestAndResponseObject(req, bodyChunks, options, res, dataChunks, rec_options.gunzip);
          if(out.reqheaders) {
          ...
          out = generateRequestAndResponse(req, bodyChunks, options, res, dataChunks, rec_options.gunzip);
        }
