...
function record(recOptions) {
  ...
  interceptor.on('request', async function ({ request: mswRequest, requestId }) {
    ...
    req.on('response', function () {
      ...
      interceptor.once('response', async function ({ response, isMockedResponse }) {
        ...
        // create deferred promise to await the response body: https://github.com/mswjs/interceptors/issues/445
        // TODO: handle reject, maybe use a more robust deferred library 
        const { promise, resolve } = common.createDeferredPromise()
    ...
    // TODO: refactor the recorder, we no longer need a ll the listeners and can just record the request we get from MSW
    req.emit('response')
  })
}
