...
// per best practices, only emit a warning once for each unique header
// https://nodejs.org/api/process.html#process_avoiding_duplicate_warnings
const warnedDupHeaders = new Set()
function emitDupHeaderWarning(headerName) {
  if (!warnedDupHeaders.has(headerName)) {
    process.emitWarning(
      `Duplicate header provided in request: ${headerName}. Only the last value can be matched.`,
      'DuplicateHeaderWarning',
      'NOCK1969'
    )
    warnedDupHeaders.add(headerName)
  }
}
