...
var normalizeRequestOptions = function(options) {
  if (options._key) {
    // the most interceptor info is contained in _key
    var parts = /(\w+) (\w+):\/\/([^:]+)\/?(.*)?:(\d+)\/?(.*)/.exec(options._key);
    options.method = parts[1];
    options.proto = parts[2];
    options.hostname = parts[3];
    options.port = parts[4];
    options.path = parts[5];
  } else {
