...
var isBinaryBuffer = function(buffer) {
  ...
  var compareBuffers = function(lhs, rhs) {
    ...
    for(var i = 0; i < lhs.length; ++i) {
      // This line is not covered. It seems possible there is no binary buffer
      // which utf-encodes to something of the same length but with different
      // characters.
      if(lhs[i] !== rhs[i]) {
        return false;
      }
...
var mergeChunks = function(chunks) {
  ...
  if(!areBuffers) {
    ...
    // This line is not covered. In reality is this ever called with strings
    // instead of buffers?
    return chunks.join('');
  }
