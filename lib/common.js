...
var headerKeysToLowerCase = function(headers) {
  if(!headers) {
    return headers;
  }

  //  For each key in the header, delete its value and reinsert it with lower-case key.
  var lowerCaseHeaders = {};
  _(headers).keys().each(function(key) {
    var lowerCaseKey = key.toLowerCase();
    if(!_.isUndefined(lowerCaseHeaders[lowerCaseKey])) {
      throw new Error('Failed to convert header keys to lower case due to key conflict: ' + lowerCaseKey);
    }
    lowerCaseHeaders[lowerCaseKey] = headers[key];
  });
...
exports.headerKeysToLowerCase = headerKeysToLowerCase;
