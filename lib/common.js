...
function overrideRequests(newRequest) {
  ...
  fetchInterceptor.on('request', function ({ request, requestId }) {
    const req = newRequest('msw', undefined, { request })
    req.end();
    request.respondWith(new Response(req.interceptors[0].body, { status: 200 }))
  })
}
