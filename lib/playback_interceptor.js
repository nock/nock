...
function parseJSONRequestBody(request, requestBody) {
  if (!requestBody || !common.isJSONContent(request.headers)) {
    return requestBody
  }

  if (common.contentEncoding(request.headers, 'gzip')) {
    requestBody = String(zlib.gunzipSync(Buffer.from(requestBody, 'hex')))
  } else if (common.contentEncoding(request.headers, 'deflate')) {
...
async function playbackInterceptor({
  ...
  interceptor.scope.emit('request', request, interceptor, requestBodyString)
  ...
  if (typeof interceptor.errorMessage !== 'undefined') {
    ...
    await new Promise(resolve => common.setTimeout(resolve, interceptor.delayBodyInMs))
    throw error
  }
  ...
  if (interceptor.replyFunction) {
    const parsedRequestBody = parseJSONRequestBody(request, requestBodyString)
    ...
    return Promise.resolve(fn.call(interceptor, url.pathname, parsedRequestBody))
  ...
  function continueWithFullResponse(fullReplyResult) {
    const [status, responseBody, rawHeaders] = parseFullReplyResult(fullReplyResult)
  ...
  function prepareResponseHeaders(body, responseHeaders) {
    ...
    common.forEachHeader([...interceptor.rawHeaders, ...responseHeaders], (value, fieldName, i) => {
      rawHeaders.push([fieldName, value])
    })

    rawHeaders.push(
      ...selectDefaultHeaders([...interceptor.rawHeaders, ...responseHeaders], defaultHeaders),
    )
    ...
    for (let i = 0; i < rawHeaders.length; i++) {
      ...
      if (typeof value === 'function') {
        rawHeaders[i][1] = value(request, undefined, body)
      }
    }
  ...
  function continueWithResponseBody(rawBody, fullResponseStatus, fullResponseRawHeaders = []) {
    ...
    bodyAsStream.on('end', function () {
      readable.push(null)
      interceptor.scope.emit('replied', request, interceptor)
    })
    ...
    const hasBody = FetchResponse.isResponseWithBody(status) && rawBody !== undefined
    return new Response(hasBody ? readable : null, { status, statusText: STATUS_CODES[status], headers })
  }
}
