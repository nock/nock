...
const { responseStatusCodesWithoutBody } = require('./create_response')
...
function playbackInterceptor({
  ...
  function continueWithResponseBody(rawBody) {
    if (
      responseStatusCodesWithoutBody.includes(response.statusCode) &&
      !isBodyEmpty(rawBody)
    ) {
      // The underlying request interceptor represents mocked responses as a Fetch API
      // `Response`, whose spec forbids a body on these status codes. Any bytes given here
      // would silently be dropped before reaching the client, so fail loudly instead.
      // https://github.com/nock/nock/issues/3000
      req.destroy(
        new Error(
          `Nock: a response body was provided for a ${response.statusCode} response, but ` +
            `responses with status ${responseStatusCodesWithoutBody.join(', ')} cannot have a body. ` +
            'Remove the body from this `.reply()` call.',
        ),
      )
      return
    }
