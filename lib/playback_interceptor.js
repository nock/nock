...
async function playbackInterceptor({
  ...
  if (typeof interceptor.errorMessage !== 'undefined') {
    ...
    await new Promise(r => common.setTimeout(r, interceptor.delayBodyInMs))
    throw error
  }
  ...
  function continueWithFullResponse(fullReplyResult) {
    let status, responseBody, rawHeaders
    try {
      [status, responseBody, rawHeaders] = parseFullReplyResult(fullReplyResult)
    } catch (err) {
      throw err
    }
  ...
  function prepareResponseHeaders(body, responseHeaders) {
    ...
    for (let i = 0; i < rawHeaders.length; i++) {
      const [fieldName, value] = rawHeaders[i]
