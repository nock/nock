  ...
  const end = function(cb) {
    ...
    function continueWithResponseBody(err, responseBody) {
      ...
      for (let i = 1, len = response.rawHeaders.length; i < len; i = i + 2) {
        const value = response.rawHeaders[i]
        if (typeof value === 'function') {
          response.rawHeaders[i] = value(req, response, responseBody)
        }
      }
...
function selectDefaultHeaders(existingHeaders, defaultHeaders) {
  ...
  for (let i = 0, len = existingHeaders.length; i < len; i = i + 2) {
    definedHeaders.add(existingHeaders[i].toLowerCase())
  }
  for (let i = 0, len = defaultHeaders.length; i < len; i = i + 2) {
    if (!definedHeaders.has(defaultHeaders[i].toLowerCase())) {
      result.push(defaultHeaders[i], defaultHeaders[i + 1])
    }
  }
