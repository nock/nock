...
function RequestOverrider(req, options, interceptors, remove, cb) {
  ...
  emitError = function(error) {
    ...
    function continueWithResponseBody(err, responseBody) {
      ...
      // Evaluate functional rawHeaders.
      Object.keys(response.rawHeaders).forEach(function(key) {
        var value = response.rawHeaders[key];

        if (typeof value === "function") {
          response.rawHeaders[key] = value(req, response, responseBody);
        }
      });
