...
module.exports = class Interceptor {
  ...
  /**
   * Start remembering requests.
   *
   * @param  {integer | null} maxRememberedRequests - Number of requests to remember.
   * Defaults to null, which remembers all of them.
   */
  remember(maxRememberedRequests = null) {
    this.maxRememberedRequests = maxRememberedRequests
    return this
  }

  /**
   * Clear the request history, to clean up after a test for the next test to work
   * properly.
   */
  clearRequestHistory() {
    this.requests = []
  ...
  rememberRequest(req) {
    if (this.maxRememberedRequests != null && this.requests.length >= this.maxRememberedRequests) {
      this.scope.logger('Maximum number of remembered requests reached')
      return
    }
