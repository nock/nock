        ...
        if (_.size(this.queries) !== _.size(queries)) {
          ...
          _.forOwn(queries, function matchOneKeyVal(val, key) {
            ...
            if (val === undefined || expVal === undefined) {
              ...
              // TODO: better array comparison. Order shouldn't matter.
              // The `qs` lib groups duplicate keys into an array of values, the current
              // implementation will not match: `/path?foo=one&foo=two` - `/path?foo=two&foo=one`
              // Before picking this up, #507 should be resolved.
