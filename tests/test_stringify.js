...
describe('stringify', () => {
    ...
    it('should handle a nested circular reference', () => {
        ...
        expect(stringify(obj)).to.equal('{"name":"parent","child":{"name":"child","parent":"[Circular ~]"}}')
    })
    ...
    it('should handle a deeply nested circular reference with path notation', () => {
        ...
        expect(stringify(obj)).to.equal('{"level1":{"level2":{"level3":{"self":"[Circular ~.level1]"}}}}')
    })

    it('should handle multiple references to the same object', () => {
        const shared = { value: 'shared' }
        const obj = { a: shared, b: shared }
        expect(stringify(obj)).to.equal('{"a":{"value":"shared"},"b":{"value":"shared"}}')
    })
    ...
    it('should handle an object with circular references in different properties', () => {
        ...
        obj.a.self = obj.a
        obj.b.self = obj.b
        expect(stringify(obj)).to.equal('{"a":{"self":"[Circular ~.a]"},"b":{"self":"[Circular ~.b]"}}')
    })
