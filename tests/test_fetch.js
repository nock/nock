...
describe('Native Fetch', () => {
  ...
  describe('redirect', () => {
    ...
    it('should follows a mocked redirect to the original server', async () => {
      nock(origin, { allowUnmocked: true }).get('/').reply(302, '', { Location: `${origin}/redirected` })
    ...
    it.skip('should follows a mocked relative redirect to the original server', async () => {
      nock(origin, { allowUnmocked: true }).get('/').reply(302, '', { Location: `/redirected` })
    ...
    it('should follows a mocked redirect to a mocked response', async () => {
      nock(origin).get('/').reply(302, '', { Location: `${origin}/redirected` })
    ...
    it('should throws a network error on a redirect for a request with "error" redirect mode', async () => {
      ...
      await assertRejects(fetch(origin, { redirect: 'error' }), /Failed to fetch/)
    });

    it('should throws a network error on a non-303 redirect with a body', async () => {
      nock(origin).post('/').reply(302, '', { Location: `${origin}/redirected` })

      await assertRejects(fetch(origin, { method: 'POST', body: 'Hello' }), /Failed to fetch/)
    });
    ...
    it('should throws on a redirect with credentials for a "cors" request', async () => {
      nock(origin).get('/').reply(302, '', { Location: `http://user:password@localhost` })
    ...
    it('should coerces a 301/302 redirect for a POST request to a GET request', async () => {
      ...
      expect(headers).to.deep.eq({ 'x-other-header': 'value', host: new URL(origin).host })
    ...
    it('should coerces a 303 redirect to a non-HEAD/GET request to a GET request', async () => {
      let body, headers;
      nock(origin).post('/').reply(303, '', { Location: `${origin}/redirected` })
      nock(origin)
        .get('/redirected')
        .reply(200, function (uri, requestBody) {
          body = requestBody
    ...
    it('should deletes sensitive request headers for a cross-origin redirect', async () => {
      let body, headers;
      nock(origin).get('/').reply(303, '', { Location: `https://anotherhost.com/redirected` })
      ...
      expect(response.status).to.eq(200)
      expect(headers).to.deep.eq({ 'x-other-header': 'value', host: 'anotherhost.com' })
      expect(body).to.be.empty()
    });
  });
})
